...
source("../utils/cvms/plotting_utilities.R")
source("../utils/cvms/helpers.R")

plot_factors_comparison <- function(conf_matrix,
                                  target_col = "Target",
  ...
  if (isTRUE(add_sums)){
    ...
    sum_data[nrow(sum_data), "N_text"] <- ifelse(isTRUE(counts_on_top), sum_data[nrow(sum_data), "N"], "")
